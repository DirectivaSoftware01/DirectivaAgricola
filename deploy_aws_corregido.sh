...
# Script para desplegar en AWS EC2 con Amazon Linux (corregido)
# IP de la instancia EC2
EC2_IP="54.212.80.37"
KEY_FILE="directiva-agricola-key.pem"

echo "🚀 Iniciando despliegue en AWS EC2 (Amazon Linux) - Versión Corregida..."
echo "📍 IP de la instancia: $EC2_IP"
echo "👤 Usuario: ec2-user"

# Verificar conectividad SSH
echo "🔐 Verificando acceso SSH..."
if ! ssh -i "$KEY_FILE" -o ConnectTimeout=10 -o StrictHostKeyChecking=no ec2-user@$EC2_IP 'echo "Conexión SSH exitosa"' 2>/dev/null; then
...
ssh -i "$KEY_FILE" -o StrictHostKeyChecking=no ec2-user@$EC2_IP << 'EOF'
    ...
    echo "🚀 Iniciando despliegue en EC2 (Amazon Linux) - Versión Corregida..."
    
    # Actualizar sistema
    echo "📦 Actualizando sistema..."
    sudo dnf update -y
    
    # Instalar Python 3.9 (disponible en Amazon Linux 2023)
    echo "🐍 Instalando Python 3.9..."
    sudo dnf install -y python3.9 python3.9-pip python3.9-devel
    
    # Instalar dependencias del sistema
    echo "📚 Instalando dependencias del sistema..."
    sudo dnf install -y postgresql postgresql-devel gcc gcc-c++ nginx git curl
    
    # Crear directorio del proyecto
    echo "📁 Configurando directorio del proyecto..."
    sudo mkdir -p /var/www/directiva_agricola
    sudo chown ec2-user:ec2-user /var/www/directiva_agricola
    ...
    sudo chown -R ec2-user:ec2-user /var/www/directiva_agricola
...
"
    ...
    python3.9 manage.py migrate --settings=directiva_agricola.settings_production --database=administracion
    python3.9 manage.py migrate --settings=directiva_agricola.settings_production --database=default
    
    # Recopilar archivos estáticos
    echo "📦 Recopilando archivos estáticos..."
    python3.9 manage.py collectstatic --noinput --settings=directiva_agricola.settings_production
    
    # Crear usuarios
    echo "👤 Creando usuarios..."
    python3.9 manage.py shell --settings=directiva_agricola.settings_production << 'EOF'
...
EOL
    ...
    sudo tee /etc/nginx/conf.d/directiva-agricola.conf > /dev/null << 'EOL'
server {
    listen 80;
...
EOL
    ...
    python3.9 manage.py crear_empresa_postgresql \
...
echo "   ssh -i $KEY_FILE ec2-user@54.212.80.37"
