...
# Script para corregir la configuración de la base de datos
...
print_status "Verificando usuario directiva_user..."
ssh $SERVER_USER@$SERVER_IP "mysql -u root -p'$DB_PASSWORD' -e 'SELECT User, Host FROM mysql.user WHERE User=\"directiva_user\";'"

print_status "Actualizando contraseña del usuario directiva_user..."
ssh $SERVER_USER@$SERVER_IP "mysql -u root -p'$DB_PASSWORD' -e 'ALTER USER \"directiva_user\"@\"localhost\" IDENTIFIED BY \"Directiva2024!\";'"
ssh $SERVER_USER@$SERVER_IP "mysql -u root -p'$DB_PASSWORD' -e 'FLUSH PRIVILEGES;'"

print_status "Probando conexión con el usuario directiva_user..."
ssh $SERVER_USER@$SERVER_IP "mysql -u directiva_user -p'Directiva2024!' -e 'SHOW DATABASES;'"
...
print_status "✅ Configuración de base de datos corregida!"
