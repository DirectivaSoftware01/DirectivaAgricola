...
# Script para desplegar en AWS EC2 con Amazon Linux
# IP de la instancia EC2
EC2_IP="54.212.80.37"
KEY_FILE="directiva-agricola-key.pem"

echo "🚀 Iniciando despliegue en AWS EC2 (Amazon Linux)..."
...
ssh -i "$KEY_FILE" -o StrictHostKeyChecking=no ec2-user@$EC2_IP << 'EOF'
    ...
    sudo yum install -y python3.13 python3.13-pip python3.13-devel
    
    # Instalar dependencias del sistema
    echo "📚 Instalando dependencias del sistema..."
    sudo yum install -y postgresql postgresql-devel gcc gcc-c++ nginx git curl
